...
def handle_user_data():
    ...
    if request.method == 'POST':
        ...
        print (disbase)
        print(fletor)
        resultado0, resultado1, resultado2, resultado3 = diagrama_magnel(areabruta, distopo, disbase, inercia, tensmax, tensmin, fletor)
    #resultMagnel()
...
def diagrama_magnel(a_c, y_t, y_b, i_c, sigma_max, sigma_min, m_sd_inicial):
    ...
    f = open("/Users/lilianevale/Documents/UFG/react/Tutorial-Project/client/public/calc.txt", "w")
    f.write( eq_0+"\n")
    f.write( eq_1+"\n")
    f.write( eq_2+"\n")
    f.write( eq_3+"\n")
...
def handle_user_data1():
    ...
    if request.method == 'POST':
        ...
        deltaperc, p_it1, sigma_pit1 = perda_deformacao_imediata_concreto_pre_tracao(e_scp*1E3, e_ccp*1E3, p_it0, sigma_pit0*1E3, a_slcp*1E-4, a_c*1E-4, i_c*1E-8, e_p*1E-2, m_gpp)       
    #resultMagnel()
...
def handle_user_data2():
    ...
    if request.method == 'POST':
        ...
        deltaperc, p_it1, sigma_pit1 = perda_deslizamento_ancoragem(p_it0, sigma_pit0*1E3, a_slcp*1E-4, l_0, Delta_anc*1E-3, e_scp*1E3)    #resultMagnel()
        print(f"resultado: {deltaperc}")
        print(f"resultado: {p_it1}")
        print(f"resultado: {sigma_pit1}")
...
def handle_user_data3():
    ...
    if request.method == 'POST':
        ...
        eps_cs, deltaperc, p_it1, sigma_pit1 = perda_retracao_concreto(p_it0, sigma_pit0*1E3, a_slcp*1E-4, u, abat*1E-2, a_c*1E-4, mu_ar*1E-2, t_0, t_1, temp, e_scp*1E3)      
        print(f"resultado: {deltaperc}")
        print(f"resultado: {p_it1}")
        print(f"resultado: {sigma_pit1}")
...
def handle_user_data4():
    ...
    if request.method == 'POST':
        ...
        sigma_t, sigma_b = tensao_topo_base(a_c*1E-4, i_c*1E-8, y_t*1E-2, y_b*1E-2, e_p*1E-2, p_id, m_sd)     
...
def handle_user_data5():
  ...
  if request.method == 'POST':
        ...
        sigma_t_vazio, sigma_b_vazio, sigma_t_serv, sigma_b_serv = tensoes_vao_completo(df, a_c*1E-4, i_c*1E-8, y_t*1E-2, y_b*1E-2)     
...
def handle_user_data6():
    ...
    if request.method == 'POST':
        ...
        deltaperc, p_it1, sigma_pit1, psi, psi_1000 = perda_relax_armadura(p_it0, sigma_pit0*1E3, t_0, t_1, temp, f_pk*1E3, a_slcp*1E-4, tipo_armadura, tipo_aco)
        # Perform any necessary backend processing with the received data
    elif request.method == 'GET':
...
def handle_user_data7():
    ...
    if request.method == 'POST':
        ...
        phi, deltaperc, p_it1, sigma_pit1 = perda_fluencia_concreto(p_it0, sigma_pit0*1E3, a_slcp*1E-4, a_c*1E-4, mu_ar*1E-2, abatimento*1E-2, umidade, tipo_endurecimento, temp, e_scp*1E3, e_ccpt1*1E3, sigma_cabo*1E3, t_0, t_1, f_ck0*1E3, f_ck*1E3)        # Perform any necessary backend processing with the received data
    elif request.method == 'GET':
...
def perda_fluencia_concreto(p_it0, sigma_pit0, a_slcp, a_c, mu_ar, abatimento, umidade, tipo_endurecimento, temp, e_scp, e_ccpt1, sigma_cabo, t_0, t_1, f_ck0, f_ck):
    ...
    if umidade  <= 90 and ((abatimento >= 0.05 and abatimento <= 0.09)):          # intervalo 0.05 <= abatimento <= 0.09
        phi_1c *= 1.00
    elif umidade  <= 90 and ((abatimento >= 0.00 and abatimento <= 0.04)):        # intervalo 0.00 <= abatimento <= 0.04
        phi_1c *= 0.75
    elif umidade  <= 90 and ((abatimento >= 0.10 and abatimento <= 0.15)):        # intervalo 10.0 <= abatimento <= 15.0
        phi_1c *= 1.25   
...
def tensoes_vao_completo(df, a_c, i_c, y_t, y_b):
    ...
    for i in range(len(df)):
        topo_vazio, base_vazio, _ = tensao_topo_base(a_c, i_c, y_t, y_b, df['e_p (cm)'][i]*1E-2, df['p_Sd t=0 (kN)'][i], df['m_Sd,gpp (kNm)'][i])
        sigma_t_vazio.append(topo_vazio)
        sigma_b_vazio.append(base_vazio)
        topo_serv, base_serv, _ = tensao_topo_base(a_c, i_c, y_t, y_b, df['e_p (cm)'][i]*1E-2, df['p_Sd t=∞ (kN)'][i], df['m_Sd,gpp (kNm)'][i]+df['m_Sd,gext (kNm)'][i]+df['m_Sd,q (kNm)'][i])
        sigma_t_serv.append(topo_serv)
        sigma_b_serv.append(base_serv)
    
    # Grafico 1 - ATO
    fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True, sharey=True, figsize=(8, 10))
    ax1.plot(df['x (cm)'], sigma_t_vazio, color='blue', linestyle='-', linewidth=2)
    ax1.plot(df['x (cm)'], sigma_b_vazio, color='red', linestyle='-', linewidth=2)
    ...
    # Grafico 2 - SERV
    ax2.plot(df['x (cm)'], sigma_t_serv, color='blue', linestyle='-', linewidth=2)
    ax2.plot(df['x (cm)'], sigma_b_serv, color='red', linestyle='-', linewidth=2)
...
def tensao_topo_base(a_c, i_c, y_t, y_b, e_p, p_sd, m_sd):
    ...
    ax.fill([250, 250, x1, x2, 250], [0, ((y_t + y_b)*100), y1, y2, 0], color='#C7C7C7', linestyle='-', linewidth=2)
    ax.tick_params(axis='both', which='both', bottom=False, top=False, left=False, right=False, labelbottom=False, labelleft=False)
    ax.set_xlim(-10, 420)
    ax.set_title('Tensões na seção transversal', size=16)  
    ax.text(-5, (y_t + y_b) * 100, f'$\sigma_t=${sigma_t:.2f} kPa', ha='left', va='center', size=14)
    ax.text(-5, 0, f'$\sigma_b=${sigma_b:.2f} kPa', ha='left', va='center', size=14)
   
    fig.savefig("/Users/lilianevale/Documents/UFG/react/Tutorial-Project/client/src/pages/teste.png", dpi=300, bbox_inches='tight')
    fig.show
...
def perda_retracao_concreto(p_it0, sigma_pit0, a_slcp, umidade, abatimento, a_c, mu_ar, t_0, t_1, temp, e_scp):
    ...
    epsilon_1s = -8.09 + (umidade / 15) - (umidade  ** 2 / 2284) - (umidade  ** 3 / 133765) + (umidade  ** 4 / 7608150)
    epsilon_1s /= -1E4 
    if umidade  <= 90 and ((abatimento >= 0.05 and abatimento <= 0.09)):          # intervalo 0.05 <= abatimento <= 0.09
        epsilon_1s *= 1.00
    elif umidade  <= 90 and ((abatimento >= 0.00 and abatimento <= 0.04)):        # intervalo 0.00 <= abatimento <= 0.04
